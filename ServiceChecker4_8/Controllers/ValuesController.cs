...
namespace ServiceChecker4_8.Controllers
{
    ...
    public class ValuesController : ApiController
    {
        ...
        public HttpResponseMessage Post([FromBody] Parameters parameters)
        {
            ...
            using (ChannelFactory<BilesikKutukSorgulaKimlikNoServis> channelFactory = new ChannelFactory<BilesikKutukSorgulaKimlikNoServis>("BindingBilesikKutukSorgulaKimlikNoServis", new EndpointAddress(KPSConfiguration.Instance.EndPoint)))
            {
                channelFactory.Credentials.SupportInteractive = false;

                channelFactory.ConfigureChannelFactory();
                BilesikKutukSorgulaKimlikNoServis servis = channelFactory.CreateChannelWithIssuedToken(KPSServiceFactory.Instance.CreateToken(KPSConfiguration.Instance.Username));
                ...
                BilesikKutukBilgileriSonucu sonuc = servis.Sorgula(list);

                if (sonuc.HataBilgisi == null)
                {
                    if (sonuc.SorguSonucu.Count > 0)
                    {
                        if (sonuc.SorguSonucu[0].KimlikNo != null)
                        {
                            if (sonuc.SorguSonucu[0].TCVatandasiKisiKutukleri.KisiBilgisi != null)
                            {
                                if (sonuc.SorguSonucu[0].TCVatandasiKisiKutukleri.KisiBilgisi.HataBilgisi == null)
                                {
                                    result.TCKimlikNo = sonuc.SorguSonucu[0].TCVatandasiKisiKutukleri.KisiBilgisi.TCKimlikNo != null ? sonuc.SorguSonucu[0].TCVatandasiKisiKutukleri.KisiBilgisi.TCKimlikNo.ToString() : "";
                ...
                listAddress.Add(new KimlikNoileAdresSorguKriteri() { KimlikNo = long.Parse(parameters.sorgulananTC),
                    DogumAy = int.Parse(parameters.dogumAy),
                    DogumGun = int.Parse(parameters.dogumGun),
                    DogumYil = int.Parse(parameters.dogumYil)
                });
